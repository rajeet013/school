const Details = () => {
  return (
    <div className="flex flex-col items-start justify-center min-[1260px]:flex-row bg-white p-15 gap-15 min-[1260px]:h-100 md:px-60">
      <div>
        <p className="font-bold  text-3xl text-black mb-4">
          Detail About How To Apply
        </p>
        <p className="text-[#6B6B96] leading-relaxed">
          If you would like to study in the university in the heart of the city
          that focus on chaning the world for better to morrow, you’re choosin
          the right place. We do not use special formulas to select students. We
          look at every single applicant’s application, academic and personal,
          to select students who suit to our community with a full range of
          backgrounds. If you would like to study in the university in the heart
          of the city.
        </p>
      </div>
      <div>
        <div className="font-bold  text-3xl text-white min-[1260px]:mb-11" />
        <p className="text-[#6B6B96] leading-relaxed mb-4">
          If you would like to study in the university in the heart of the city
          that focus on chaning the world for better to morrow, you’re choosin
          the right place. We do not use special formulas to select students. We
          look at every single applicant’s application, academic and personal,
          to select students who suit to our community.
        </p>
      </div>
    </div>
  );
};

export default Details;
